...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"AES": [128, 192, 256], "Blowfish": [112, 224, 448], "DES3": [112]},
            "ProposedCiphers:AES:[128,192,256],Blowfish:[112,224,448],DES3:[112]",
        ),
    ],
)
...
@pytest.mark.parametrize("value, message", [(101, "DHMKE:101"), (8946513853, "DHMKE:8946513853")])
def test_generate_dhm_request(value, message):
    """Test DHMKE message generation"""
    assert generate_dhm_request(value) == message


@pytest.mark.parametrize("message, value", [("DHMKE:101", 101), ("DHMKE:8946513853", 8946513853)])
...
@pytest.mark.parametrize(
    ...
    [
        ("123456789", "DES3", 112, DES3, b"123456789", b"23456789"),
        (
